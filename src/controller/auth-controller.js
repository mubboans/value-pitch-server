...
const Login = TryCatch(async (req, res, next) => {
    ...
    if (bcrypt.compareSync(hashPass, userRecord.password)) {
        if (!userRecord.isActive) return next(new CustomError('It seems your account is Freeze', 403));
        if (userRecord.deletionDate) return next(new CustomError('It seem your account is deleted', 403));
    ...
    return next(new CustomError("Account detail not match", 404));
})
